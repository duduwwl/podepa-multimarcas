...
export default function Storefront({ mode = "home" }: { mode?: "home" | "catalog" } = {}) {
  ...
  function openProduct(product:CatalogProduct) { setDetailProduct(product); setDetailImage(0); }
  ...
  return <main>
    ...
    {detailProduct&&<><button className="product-modal-backdrop" onClick={()=>setDetailProduct(null)} aria-label="Fechar detalhes do produto"/><section className="product-modal" role="dialog" aria-modal="true" aria-labelledby="product-detail-title"><button className="product-modal-close" onClick={()=>setDetailProduct(null)} aria-label="Fechar"><X/></button><div className={`product-modal-gallery ${detailProduct.id==="camiseta-casa-blanca"?"white-backdrop":""}`}><img src={detailProduct.images[detailImage]} alt={`${detailProduct.name}, imagem ${detailImage+1}`}/>{detailProduct.images.length>1&&<><button className="modal-prev" onClick={()=>setDetailImage((detailImage-1+detailProduct.images.length)%detailProduct.images.length)} aria-label="Imagem anterior"><ChevronLeft/></button><button className="modal-next" onClick={()=>setDetailImage((detailImage+1)%detailProduct.images.length)} aria-label="Próxima imagem"><ChevronRight/></button><div className="modal-thumbs">{detailProduct.images.map((image,index)=><button key={image} className={detailImage===index?"active":""} onClick={()=>setDetailImage(index)} aria-label={`Ver imagem ${index+1}`}><img src={image} alt=""/></button>)}</div></>}</div><div className="product-modal-copy"><p className="eyebrow">{detailProduct.category} • {detailProduct.badge||"ESTOQUE DISPONÍVEL"}</p><h2 id="product-detail-title">{detailProduct.name}</h2><div className="modal-price"><strong>{money(detailProduct.price)}</strong>{detailProduct.compareAt&&<del>{money(detailProduct.compareAt)}</del>}</div><p className="modal-description">{detailProduct.description}</p><div className="spec-grid"><span><Tag/><small>Código</small><strong>{detailProduct.sku}</strong></span><span><Ruler/><small>Tamanhos</small><strong>{detailProduct.sizes.join(" • ")}</strong></span><span><PackageCheck/><small>Disponibilidade</small><strong>{detailProduct.initialStock} unidades</strong></span><span><Truck/><small>Envio</small><strong>Todo o Brasil</strong></span></div><label className="modal-size">Escolha o tamanho<select value={selectedSizes[detailProduct.id]||detailProduct.sizes[0]} onChange={(event)=>setSelectedSizes((current)=>({...current,[detailProduct.id]:event.target.value}))}>{detailProduct.sizes.map((size)=><option key={size}>{size}</option>)}</select></label><button className="modal-add" onClick={()=>{addToCart(detailProduct);setDetailProduct(null)}}>Adicionar à sacola <ShoppingBag/></button><small className="modal-note"><MapPin/> Retirada grátis em Lavras ou entrega calculada pelo CEP.</small></div></section></>}
