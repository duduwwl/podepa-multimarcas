...
import { ArrowRight, Camera, CheckCircle2, ChevronLeft, ChevronRight, CreditCard, Hash, Heart, MapPin, Menu, MessageCircle, Minus, PackageCheck, Plus, QrCode, Ruler, Search, ShieldCheck, ShoppingBag, Sparkles, Tag, Trash2, Truck, X, ZoomIn } from "lucide-react";
import { FormEvent, useEffect, useMemo, useState } from "react";
import { catalog, CatalogProduct, productById } from "../lib/catalog";
import { apiUrl, appHref, assetUrl } from "../lib/runtime";
...
export default function Storefront({ mode = "home" }: { mode?: "home" | "catalog" } = {}) {
  ...
  async function calculateShipping(cep:string) {
    ...
    try { const response = await fetch(apiUrl(`/api/shipping?cep=${clean}&items=${Math.max(1,cartCount)}`)); const data = await response.json() as ShippingQuote & {error?:string}; if (!response.ok) throw new Error(data.error); setShipping(data); return data; }
  ...
  async function placeOrder(event:FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const submittedPaymentMethod = paymentMethod === "card" ? cardType : "pix";
      const body = { name:form.get("name"), email:form.get("email"), phone:form.get("phone"), cpf:form.get("cpf"), cep:deliveryMethod === "delivery" ? form.get("cep") : "", address, city:deliveryMethod === "delivery" ? form.get("city") : "", state:deliveryMethod === "delivery" ? form.get("state") : "", deliveryMethod, paymentMethod:submittedPaymentMethod, items:cart };
      const response = await fetch(apiUrl("/api/orders"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
      const data = await response.json() as {orderId:string;error?:string}; if (!response.ok) throw new Error(data.error);
  ...
  return <main>
    <header className="nav-shell">
      ...
      <a className="brand header-brand" href={appHref("/")} aria-label="Pode Pá Multimarcas, início"><img className="brand-top-mark" src={assetUrl("/podepa-peace-mark.png")} alt=""/><span>Pode Pá</span><small>MULTIMARCAS</small></a>
      <nav aria-label="Navegação principal"><a href={appHref("/produtos")}>Produtos</a><a href="#rodape">Sobre nós</a></nav>
      <div className="nav-actions">
        {mode === "catalog" ? <label className={`nav-search ${query ? "open" : ""}`}><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar no estoque" aria-label="Buscar no estoque" /></label> : <a className="icon-button" href={appHref("/produtos")} aria-label="Buscar produtos"><Search /></a>}
    ...
    {mode === "home" ? <>
      ...
      <section className="home-showcase">
        <div className="home-showcase-heading"><div><p className="eyebrow">ESCOLHA O SEU CORRE</p><h2>Um estilo para cada momento.</h2></div><a href={appHref("/produtos")}>Ver catálogo <ArrowRight/></a></div>
        <div className="home-showcase-grid">
          <a href={appHref("/produtos?categoria=Camisetas")}><span>01</span><img src={assetUrl("/products/camiseta-verde-frente.png")} alt="Camiseta verde Casa Blanca"/><div><small>Camisetas</small><strong>Presença em cada detalhe</strong><ArrowRight/></div></a>
          <a href={appHref("/produtos?categoria=Moletons")}><span>02</span><img src={assetUrl("/products/moletom-preto.png")} alt="Moletom Champion preto"/><div><small>Moletons</small><strong>Conforto com assinatura</strong><ArrowRight/></div></a>
          <a href={appHref("/produtos?categoria=Bermudas")}><span>03</span><img src={assetUrl("/products/bermuda-jeans-neon.png")} alt="Bermuda jeans com cordões neon"/><div><small>Bermudas</small><strong>Do básico ao marcante</strong><ArrowRight/></div></a>
        </div>
      </section>
    ...
    <footer id="rodape"><div><a className="brand footer-brand" href={appHref("/")}><span>Pode Pá</span><small>MULTIMARCAS</small></a><p>Moda masculina com identidade, atitude e peças que combinam com o seu estilo.<br/>De Lavras para o Brasil.</p></div><div><strong>Atendimento</strong><a href="https://wa.me/5535984649336" target="_blank" rel="noreferrer">WhatsApp</a><a href="https://www.instagram.com/loja_podepa/" target="_blank" rel="noreferrer">Instagram @loja_podepa</a></div><div><strong>Loja</strong><a href={appHref("/produtos")}>Produtos</a><a href={appHref("/gerencia")}>Área da gerência</a></div></footer>
    ...
    {detailProduct&&<>
      ...
      <section className="product-modal" role="dialog" aria-modal="true" aria-labelledby="product-detail-title">
        ...
        <div className={`product-modal-gallery ${detailProduct.id==="camiseta-casa-blanca"||detailProduct.category==="Kits"?"white-backdrop":""}`}>
          <img src={assetUrl(detailProduct.images[detailImage])} alt={`${detailProduct.name}, imagem ${detailImage+1}`}/>
          {detailProduct.images.length>1&&!detailProduct.colors&&<>
            ...
            <div className="modal-thumbs">{detailProduct.images.map((image,index)=><button key={image} className={detailImage===index?"active":""} onClick={()=>setDetailVariant(detailProduct,index)} aria-label={`Ver imagem ${index+1}`}><img src={assetUrl(image)} alt=""/></button>)}</div>
          </>}
        </div>
